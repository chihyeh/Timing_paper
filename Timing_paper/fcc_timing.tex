...
\begin{abstract}
This paper explores the idea of using timing layers with a few tens of picoseconds resolution for 
future particle-collision experiments.
The presented studies show the effect of timing layers on identification 
of separate Standard Model particles,  as well as illustrate the benefit of using
such detectors for detection of new event signatures beyound  the Standard Model. 
\end{abstract}
...
investigate the capabilities of timing layers for identification of heavy stable particles which may be produced beyond the 
Standard Model (BSM). We hope such studies can help shape the requirements for future calorimeters, which were already
outlined in the CPAD report \cite{ahmed2019new} which states that one critical needs is to develop calorimeters with picosecond 
time  resolution. 
