...
for Sort_particle in range(1,8):
    ...
    for Bin in range(1,10):
        ...
        X_PT.append(-2.25+0.5*Bin)
        X_Error.append(0)
        Y_Error.append(0)

        if(Bin<9):
            H1_Projection = h1.ProjectionX("1",Bin,Bin,"")
            H2_Projection = h2.ProjectionX("2",Bin,Bin,"")
            if(H1_Projection.Integral()!=0):
                H1_Projection.Sumw2()
                H1_Projection.Scale(1/H1_Projection.Integral())
                Fraction_QQ.append(H1_Projection.GetBinContent(Sort_particle))
            ...
            if(H2_Projection.Integral()!=0):
                ...
                Fraction_WW.append(H2_Projection.GetBinContent(Sort_particle))
            else:
    ...
    gr_QQ = TGraphErrors(9,X_PT,Fraction_QQ,X_Error,Y_Error)
    ...
    gr_WW = TGraphErrors(9,X_PT,Fraction_WW,X_Error,Y_Error)
    ...
    gr_QQ.GetXaxis().SetLimits(-2,2.5)
