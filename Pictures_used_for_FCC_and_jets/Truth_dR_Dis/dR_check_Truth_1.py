...
for E in range(4):
    ...
    myTree_QQ = f1.Get("BDT_variables")
    myTree_WW = f2.Get("BDT_variables")
    for j in range(2):
        h1  = TH1F ("QQ_plot0","QQ_plot0",20,0,0.5)
        h2  = TH1F ("WW_plot0","WW_plot0",20,0,0.5)
        h3  = TH1F ("QQ_plot1","QQ_plot1",20,0,0.5)
        h4  = TH1F ("WW_plot1","WW_plot1",20,0,0.5)
        h5  = TH1F ("QQ_plot2","QQ_plot2",20,0,0.5)
        h6  = TH1F ("WW_plot2","WW_plot2",20,0,0.5)
        h7  = TH1F ("QQ_plot3","QQ_plot3",20,0,0.5)
        h8  = TH1F ("WW_plot3","WW_plot3",20,0,0.5)
        h9  = TH1F ("QQ_plot4","QQ_plot4",20,0,0.5)
        h10 = TH1F ("WW_plot4","WW_plot4",20,0,0.5)
        ...
        for iii in range(int(myTree_QQ.GetEntriesFast())):
            ...
            EE     = myTree_QQ.dR_Tr4T_HPt
            G      = myTree_QQ.dR_Tr0PT_HPt
            H      = myTree_QQ.dR_Tr1PT_HPt
            I      = myTree_QQ.dR_Tr2PT_HPt
            J      = myTree_QQ.dR_Tr3PT_HPt
            K      = myTree_QQ.dR_Tr4PT_HPt
            if(j==0):
                if(A < 0.5):
                    h1.Fill(A)
                else:
        ...
        for iii in range(int(myTree_WW.GetEntriesFast())):
            ...
            A      = myTree_WW.dR_Tr0T_HPt
            B      = myTree_WW.dR_Tr1T_HPt
            C      = myTree_WW.dR_Tr2T_HPt
            D      = myTree_WW.dR_Tr3T_HPt
            EE     = myTree_WW.dR_Tr4T_HPt
            G      = myTree_WW.dR_Tr0PT_HPt
            H      = myTree_WW.dR_Tr1PT_HPt
            I      = myTree_WW.dR_Tr2PT_HPt
            J      = myTree_WW.dR_Tr3PT_HPt
            K      = myTree_WW.dR_Tr4PT_HPt
            if(j==0):
                if(A < 0.5):
                    h2.Fill(A)
                else:
...
#       leg.AddEntry(h2,"Z'("+str(energy[E])+"TeV)#rightarrowW^{+}W^{-}#rightarrow2 subjets","l")
        ...
        c.Print(str(energy[E])+"TeV/dR_"+str(list_PT_T[j])+"_"+str(energy[E])+"TeV_Truth.pdf")
